...
// actually be reached with the jump the crab has — and whether a level can go on a dare card.
//
...
function checkLevel(level, index) {
  ...
  if (!/^[a-z0-9-]+$/.test(level.id)) err(`id "${level.id}" must be lowercase letters, digits and dashes — it goes in a link`);
  const missing = missingGlyphs(`${index + 1}. ${level.name}`);
  if (missing.length) err(`the dare card font has no ${missing.map((ch) => `"${ch}"`).join(", ")} for the name`);
