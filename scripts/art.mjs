// Every picture in the game, drawn from ASCII pixel grids, plus the tiny raster and PNG encoder
// that turn them into files. No dependencies — PNGs are encoded by hand with node:zlib.
// scripts/gen-assets.mjs writes the static files; api/dare.js draws dare cards on demand.
...
// 5x7, one string per row. "·" is the mid-line separator, "." sits on the baseline.
const GLYPHS = {
  A: [".###.", "#...#", "#...#", "#####", "#...#", "#...#", "#...#"],
  B: ["####.", "#...#", "#...#", "####.", "#...#", "#...#", "####."],
  C: [".###.", "#...#", "#....", "#....", "#....", "#...#", ".###."],
...
/** Characters the pixel font can't draw — a card with any of these would silently lose them. */
export const missingGlyphs = (text) => [...new Set([...text])].filter((ch) => ch !== " " && !GLYPHS[ch]);
...
// ---------------------------------------------------------------- dare card (1200x630)
// What a shared dare link unfurls into on X, Telegram and friends: the level's location,
// the time to beat and the level's name. Drawn on demand by api/dare.js, so it stays one cheap pass.

// the part of src/themes.ts a still picture needs (the game's copy also carries light and dust)
const SKIES = {
  meadow: { sky: "#1a1c2c", stars: 46, star: "#94b0c2", celestial: "moon", ceiling: false },
  desert: { sky: "#5d275d", stars: 0, star: "#ffcd75", celestial: "sun", ceiling: false },
  cavern: { sky: "#0d0e17", stars: 34, star: "#73eff7", celestial: null, ceiling: true },
  frost: { sky: "#29366f", stars: 40, star: "#f4f4f4", celestial: "moon", ceiling: false },
  magma: { sky: "#23121e", stars: 26, star: "#ef7d57", celestial: null, ceiling: false },
};

const INK = "#0d0e17";

function shadowText(img, text, x, y, scale, color) {
  const shadow = Math.max(3, Math.round(scale * 0.6));
  drawText(img, text, x + shadow, y + shadow, scale, INK);
  return drawText(img, text, x, y, scale, color);
}
...
export function cardImage({ theme, time, subtitle }) {
  ...
  const H = 630;
  const S = 10;
  const look = SKIES[theme];
  const img = new Img(W, H);
  img.fill(0, 0, W, H, look.sky);

  const brand = 5;
  const big = 16;
  const brandY = look.ceiling ? 118 : 46;
  const askY = brandY + 7 * brand + 30;
  const timeY = askY + 7 * 6 + 22;
  const subY = timeY + 7 * big + 26;
  // a star between two digits reads as a stray pixel, not a star: keep the text block clear
  const clear = (x, y) => x > 260 && x < 940 && y > brandY - 20 && y < subY + 7 * 6 + 20;

  // same seed for every card of a location, so a level always gets the same sky
  let seed = 7 + THEMES.indexOf(theme);
  const rnd = () => (seed = (seed * 16807) % 2147483647) / 2147483647;
  for (let i = 0; i < Math.round(look.stars * 1.5); i++) {
    const large = rnd() > 0.8;
    const [sx, sy] = [Math.floor(rnd() * 120) * S, Math.floor(rnd() * 40) * S];
    if (!clear(sx, sy)) img.fill(sx, sy, S * (large ? 1 : 0.5), S * (large ? 1 : 0.5), large ? C.white : look.star);
  }
  if (look.celestial === "moon") img.blit(moonImg, 1045, 25, S / 2);
  if (look.celestial === "sun") img.blit(sunImg, 1035, 25, S / 2);

  const [farColor, farBase, farWaves, nearColor, nearBase, nearWaves, tri] = SKYLINES[theme];
  img.blit(hills(320, 96, farColor, farBase, farWaves, tri), 0, 250, 4);
  img.blit(hills(320, 64, nearColor, nearBase, nearWaves, tri), 0, 330, 4);
  if (look.ceiling) img.blit(caveCeiling, 0, -40, 4);

  const groundY = H - 16 * S;
  const top = fromGrid(`${theme}.top`, TILE_TOP, { ...TILE_PALS[theme], ...SPIKE_PAL });
  for (let x = 0; x < W; x += 16 * S) img.blit(top, x, groundY, S);

  // CRAB VS BUGS, small, so the time is what the eye lands on
  const gap = 3 * brand;
  const words = [["CRAB", "#d97757"], ["VS", C.white], ["BUGS", "#b13e53"]];
  let x = centreX(img, words.reduce((sum, [word]) => sum + textWidth(word, brand), 0) + gap * (words.length - 1));
  for (const [word, color] of words) x = shadowText(img, word, x, brandY, brand, color) - brand + gap;

  shadowText(img, "CAN YOU BEAT", centreX(img, textWidth("CAN YOU BEAT", 6)), askY, 6, C.white);
  shadowText(img, time, centreX(img, textWidth(time, big)), timeY, big, "#ffcd75");
  shadowText(img, subtitle, centreX(img, textWidth(subtitle, 6)), subY, 6, C.white);

  // the crab mid-hop at the bug it's about to squash
  img.blit(crabJump, 90, groundY - 16 * S - 70, S);
  img.blit(bugWalk1, 950, groundY - 16 * S, S);
  return img;
}
